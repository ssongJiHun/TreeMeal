...
public class FloatingActionsMenu extends ViewGroup {
	...
	private void createAddButton(Context context) {
		mAddButton = new AddFloatingActionButton(context) {
			...
			Drawable getIconDrawable() {
				// super.getIconDrawable() 부모도 호출하고 이것도하고\
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		case EXPAND_DOWN:
			...
			int labelsXNearButton = mLabelsPosition == LABELS_ON_LEFT_SIDE ? buttonsHorizontalCenter - labelsOffset : buttonsHorizontalCenter
					+ labelsOffset;
			...
			for (int i = mButtonsCount - 1; i >= 0; i--) {
				...
				if (label != null) {
					...
					Rect touchArea = new Rect(Math.min(childX, labelLeft), childY - mButtonSpacing / 2, Math.max(childX + child.getMeasuredWidth(),
							labelRight), childY + child.getMeasuredHeight() + mButtonSpacing / 2);
